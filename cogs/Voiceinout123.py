...
class Voiceinout123(commands.Cog):
    ...
    async def on_voice_state_update(self,member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        channel = self.bot.get_channel(1243243269736108172)
        if before.channel is None and after.channel is not None:
            ...
            if after.channel is not None:
                ...
                embed.add_field(name='打飛頻道', value=before.channel.name, inline=False)
                embed.add_field(name='伺服器', value=member.guild.name, inline=False)
                await channel.send(embed=embed)


async def setup(bot: commands.Bot):
    await bot.add_cog(Voiceinout123(bot))